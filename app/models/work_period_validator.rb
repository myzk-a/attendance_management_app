...
class WorkPeriodValidator < ActiveModel::EachValidator
  def validate_each(record, attribute, value)
    ...
    return if record.user_id.blank?

     # 重複する期間を検索(編集時は自期間を除いて検索)
    if record.id.present?
      not_own_periods = Work.where('user_id IN (?) AND id NOT IN (?) AND start_time < ? AND end_time > ?', record.user_id, record.id, new_end_time, new_start_time)
    else
      not_own_periods = Work.where('user_id IN (?) AND start_time < ? AND end_time > ?', record.user_id, new_end_time, new_start_time)
    end
